...
Record the approved desktop frontend technology direction while keeping
AudioCore ownership narrow. This file exists to prevent frontend framework
decisions from drifting into the native audio engine tree.

## Scope

This decision covers the product desktop frontend stack, its expected boundary
with the AudioCore Host ABI, and the first product-side scaffold shape. It does
not authorize frontend implementation inside `KivoAudioCoreCpp/`.

## Current Phase

Qt 6 is approved as the product frontend direction. AudioCore remains in a
core-only phase where UI code, product settings, media-library storage, and
installer UX are out of scope.

## Allowed

- Product repositories may build a Qt 6/QML desktop shell.
- Product code may consume stable AudioCore Host ABI headers.
- Product-side C++ may adapt ABI snapshots into Qt view models.
- AudioCore documentation may describe the boundary and required evidence.

## Forbidden

- Do not add Qt, QML, product windows, product routes, or UI build targets to
  `KivoAudioCoreCpp/`.
- Do not let QML call AudioCore native internals directly.
- Do not claim Hi-Res, DSD, DoP, exclusive, bit-perfect, or gapless support
  from product UI state alone.

## Gate

AudioCore gates must continue to pass with no Qt symbols, QML files, product
frontend targets, or UI ownership inside the core repository. Product frontend
builds must prove their own CMake/QML/screenshot checks outside AudioCore.

## Failure

If frontend work requires native evidence not exposed by the Host ABI, the
frontend must stop at an unknown/deferred state until AudioCore admits a narrow
contract slice and proves it with machine-checkable tests.

## Future Phase

Later product phases may add real playback wiring, media-library persistence,
lyrics rendering, global media controls, packaging, and commercial QA in the
frontend repository after the required ABI and validation evidence exists.

## Deferred

NAS, WebDAV, cloud drives, AI features, mobile targets, installer signing,
commercial telemetry, and support workflows remain outside this AudioCore
technology decision until separate product tickets admit them.
