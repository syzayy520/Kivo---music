# P0-C-042 CUE Continuity Header Blank Normalization — Taskbook

**Task ID:** `KIVO-AUDIO-CORE-P0-C-042-CUE-CONTINUITY-HEADER-BLANK-NORMALIZATION-TASKBOOK-001`

Implementation allowed by Owner authorization.

## Goal

Normalize the P0-C CUE continuity headers so the file banner is followed by a single blank separator before `#pragma once`, matching the cleaned CUE range and boundary header layout.
...
- No runtime logic.
- No CUE parser, gapless engine, decoder, timeline mapper, source reader, render thread, or output path logic.
...
For the six selected CUE continuity headers, normalize the banner tail so it becomes:
...
- File count remains 6 production headers touched.
...
```text
selected CUE continuity headers have a single blank separator before #pragma once
...
changed files limited to this taskbook + 6 CUE continuity headers
```

## Final classification

```text
KIVO-AUDIO-CORE-P0-C-042-CUE-CONTINUITY-HEADER-BLANK-NORMALIZATION
