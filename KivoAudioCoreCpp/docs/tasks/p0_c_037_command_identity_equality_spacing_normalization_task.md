# P0-C-037 Command Identity Equality Spacing Normalization — Taskbook

**Task ID:** `KIVO-AUDIO-CORE-P0-C-037-COMMAND-IDENTITY-EQUALITY-SPACING-NORMALIZATION-TASKBOOK-001`

Implementation allowed by Owner authorization.

## Goal

Normalize the P0-C command identity value headers so the equality operator block is visually separated from the final field block, matching the value-contract readability style already used by the command aggregate contracts and neighboring source, policy, observability, error, format, and state contract structs.
...
```text
include/kivo/core/contract/command/identity/
```
...
- No runtime logic.
- No command dispatcher, queue, scheduler, source/device mutation, thread, or output path logic.
...
Insert exactly one blank line between the final field declaration and the `[[nodiscard]] constexpr operator==` declaration in the two selected identity structs.
...
```text
selected command identity structs have one blank separator before equality operator
...
changed files limited to this taskbook + 2 command identity headers
```

## Final classification

```text
KIVO-AUDIO-CORE-P0-C-037-COMMAND-IDENTITY-EQUALITY-SPACING-NORMALIZATION
