...
if ($OpenFilePath) {
    Write-Host "OPEN_FILE_PATH: $OpenFilePath"
}
if ($CaptureDelayMs -gt 0) {
    Write-Host "CAPTURE_DELAY_MS: $CaptureDelayMs"
}
Write-Host ""
...
if ($PageKey) {
    $arguments += @("--screenshot-page", $PageKey)
}
if ($OpenFilePath) {
    $arguments += @("--open-file", $OpenFilePath)
}
if ($CaptureDelayMs -gt 0) {
    $arguments += @("--screenshot-delay-ms", [string]$CaptureDelayMs)
}
& $exePath @arguments
